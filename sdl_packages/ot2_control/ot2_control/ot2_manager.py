...
class OT2Manager(Node):
    ...
    def execute_callback(self, goal_handle):
        ...
        custom_labware_folder = str(custom_labware_folder)
        parameters_json = goal_handle.request.parameters_json
        self.get_logger().info(f"Starting OT-2 protocol: {protocol_path} with parameters: {parameters_json}")


        params = json.loads(parameters_json)


        self.status, self.current_run_id, self.current_protocol_id, self.labware = self.client.run_protocol(protocol_path=protocol_path, custom_labware_folder=custom_labware_folder, parameters=params)
