...
def run(protocol: protocol_api.ProtocolContext):
    ...
    for i in range(sample_count):
        if v_stock[i] > 0:
            ...
            right_pipette.transfer(v_stock[i], fehlings_ingredients["B1"], samples.wells()[i].top(-10), blow_out=True, blowout_location="destination well")
            large_pipettes += 1
            stock_used += v_stock[i]
        
        if v_water[i] > 0:
            #adding water to sample wells
            right_pipette.transfer(v_water[i], fehlings_ingredients["B2"], samples.wells()[i].top(-10), blow_out=True, blowout_location="destination well")
            large_pipettes += 1
            water_used += v_water[i]
        

    sample_prep_time = t_sample_prep.elapsed()
    ...
    for item_id in range(protocol.params.sample_count):
        ...
        while True:
            ...
            if free_slots:
                ...
                right_pipette.transfer(v_AB, fehlings_ingredients["B5"], fehlings_mix["A1"]) #aspirating and dispensing A part
                large_pipettes += 1
                ...
                #aspirate and dispense B part with volume check
                right_pipette.pick_up_tip()
                large_pipettes += 1

                if v_AB > right_pipette.max_volume:
                    for _ in range(int(v_AB // right_pipette.max_volume)):
                        right_pipette.aspirate(right_pipette.max_volume, fehlings_ingredients["B6"]) #aspirating B part
                        right_pipette.dispense(right_pipette.max_volume, fehlings_mix["A1"]) #dispensing B part
                        #keep going until full volume is transferred
                    remainder = v_AB % right_pipette.max_volume
                    if remainder > 0:
                        right_pipette.aspirate(remainder, fehlings_ingredients["B6"]) #aspirating B part
                        right_pipette.dispense(remainder, fehlings_mix["A1"]) #dispensing B part
                else:
                    right_pipette.aspirate(v_AB, fehlings_ingredients["B6"]) #aspirating B part
                    right_pipette.dispense(v_AB, fehlings_mix["A1"]) #dispensing B part
                ...
                #mix the fehlings solution before use

                if v_fehlings + 50 > right_pipette.max_volume:
                    right_pipette.mix(6, right_pipette.max_volume, fehlings_mix["A1"]) # mixing the fehlings solution
                else:
                    right_pipette.mix(4, v_fehlings + 50, fehlings_mix["A1"]) # mixing the fehlings solution


                #adding fehlings solution to sample with volume check

                if v_fehlings > right_pipette.max_volume:
                    for _ in range(int(v_fehlings // right_pipette.max_volume)):
                        right_pipette.aspirate(right_pipette.max_volume, fehlings_mix["A1"])  # aspirating fehlings solution
                        right_pipette.dispense(right_pipette.max_volume, samples.wells()[item_id])  # dispensing fehlings solution
                    remainder = v_fehlings % right_pipette.max_volume
                    if remainder > 0:
                        right_pipette.aspirate(remainder, fehlings_mix["A1"])  # aspirating fehlings solution
                        right_pipette.dispense(remainder, samples.wells()[item_id])  # dispensing fehlings solution
                else:
                    right_pipette.aspirate(v_fehlings, fehlings_mix["A1"])  # aspirating fehlings solution
                    right_pipette.dispense(v_fehlings, samples.wells()[item_id])  # dispensing fehlings solution


                #right_pipette.transfer(v_fehlings, fehlings_mix.wells()[item_id], samples.wells()[item_id])  # adding fehlings solution to sample
                #
                ...
                if v_fehlings > right_pipette.max_volume:
                    right_pipette.mix(6, right_pipette.max_volume, samples.wells()[item_id]) # mixing the fehlings solution
                else:
                    right_pipette.mix(4, v_fehlings, samples.wells()[item_id]) # mixing the fehlings solution


                right_pipette.blow_out()

                right_pipette.mix(6, right_pipette.max_volume, samples.wells()[item_id])  # mixing the final sample solution
                ...
                right_pipette.transfer(
                    ...
                    new_tip='never', 
                    blow_out=True, 
                    blowout_location="destination well"                      # <-- REUSE THE SAME TIP
                )
    ...
    protocol.comment(f"Sample preparation time[s]: {sample_prep_time:.2f}") 
    protocol.comment("Fehlings prep times[s]: " + ", ".join([f"{pt:.2f}" for pt in prep_times]))

    schedule_time = t_scheduling.elapsed()
    protocol.comment(f"Scheduling time[s]: {schedule_time:.2f}")
